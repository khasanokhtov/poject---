...
const MachineWorkPlanRowsAPIURL = "https://operations.cropwise.com/api/v3/machine_work_plan_rows"

// FetchAndSaveMachineWorkPlanRows - функция для загрузки и сохранения данных
func FetchAndSaveMachineWorkPlanRows(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MachineWorkPlanRowsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MachineWorkPlanRowModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения machine_work_plan_row с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные machine_work_plan_rows успешно загружены для схемы %s", schemaName)
	return nil
}
