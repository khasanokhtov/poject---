...
const MaintenanceRecordsAPIURL = "https://operations.cropwise.com/api/v3/maintenance_records"

// FetchAndSaveMaintenanceRecords - функция для загрузки и сохранения данных
func FetchAndSaveMaintenanceRecords(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MaintenanceRecordsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MaintenanceRecord `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения maintenance_record с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные maintenance_records успешно загружены для схемы %s", schemaName)
	return nil
}
