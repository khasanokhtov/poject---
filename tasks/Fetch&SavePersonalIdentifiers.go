...
const PersonalIdentifiersAPIURL = "https://operations.cropwise.com/api/v3/personal_identifiers"

// FetchAndSavePersonalIdentifiers - функция для загрузки и сохранения данных
func FetchAndSavePersonalIdentifiers(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := PersonalIdentifiersAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.PersonalIdentifier `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения personal_identifier с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные personal_identifiers успешно загружены для схемы %s", schemaName)
	return nil
}
