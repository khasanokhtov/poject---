...
const AgronomistFieldsEndpoint = "https://operations.cropwise.com/api/v3/agronomist_fields"

func FetchAndSaveAgronomistFields(token, schemaName string) error {
	...
	for {
		// Создаём запрос
		url := AgronomistFieldsEndpoint + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON
		var response struct {
			Data []models.AgronomistFieldModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения объекта ID %d: %w", item.ID, err)
			}
		}
