...
const AllowedToCropsEndpoint = "https://operations.cropwise.com/api/v3/allowed_to_crops"

func FetchAndSaveAllowedToCrops(token, schemaName string) error {
	...
	for {
		...
		url := AllowedToCropsEndpoint + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON
		var response struct {
			Data []models.AllowedToCropModel `json:"data"`
			Meta struct {
				Response struct {
