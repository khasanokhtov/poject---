...
const MaintenanceTypesAPIURL = "https://operations.cropwise.com/api/v3/maintenance_types"

// FetchAndSaveMaintenanceTypes - функция для загрузки и сохранения данных
func FetchAndSaveMaintenanceTypes(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MaintenanceTypesAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MaintenanceType `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения maintenance_type с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные maintenance_types успешно загружены для схемы %s", schemaName)
	return nil
}
