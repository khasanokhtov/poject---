...
const ProductivityEstimatesAPIURL = "https://operations.cropwise.com/api/v3/productivity_estimates"

// FetchAndSaveProductivityEstimates - функция для загрузки и сохранения данных
func FetchAndSaveProductivityEstimates(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := ProductivityEstimatesAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.ProductivityEstimate `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения productivity_estimate с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные productivity_estimates успешно загружены для схемы %s", schemaName)
	return nil
}
