...
const AvatarsEndpoint = "https://operations.cropwise.com/api/v3a/avatars"

func FetchAndSaveAvatars(token, schemaName string) error {
	...
	for {
		...
		url := AvatarsEndpoint + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON
		var response struct {
			Data []models.AvatarModel `json:"data"`
			Meta struct {
				Response struct {
