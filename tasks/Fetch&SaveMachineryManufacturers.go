...
const MachineryManufacturersAPIURL = "https://operations.cropwise.com/api/v3/machinery_manufacturers"

// FetchAndSaveMachineryManufacturers - функция для загрузки и сохранения данных
func FetchAndSaveMachineryManufacturers(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MachineryManufacturersAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MachineryManufacturerModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения machinery_manufacturer с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные machinery_manufacturers успешно загружены для схемы %s", schemaName)
	return nil
}
