...
const MaintenanceRecordRowsAPIURL = "https://operations.cropwise.com/api/v3/maintenance_record_rows"

// FetchAndSaveMaintenanceRecordRows - функция для загрузки и сохранения данных
func FetchAndSaveMaintenanceRecordRows(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MaintenanceRecordRowsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MaintenanceRecordRow `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения maintenance_record_row с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные maintenance_record_rows успешно загружены для схемы %s", schemaName)
	return nil
}
