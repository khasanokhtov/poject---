...
func FetchAndSaveDataSourceGPSLoggers(token string, schemaName string) error {
	log.Printf("Начинаем загрузку данных о GPS-логгерах для схемы: %s", schemaName)
	...
	for {
		url := DataSourceGPSLoggersAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		for _, gpsLogger := range response.Data {
			if err := database.DB.Save(&gpsLogger).Error; err != nil {
