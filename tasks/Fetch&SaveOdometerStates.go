...
const OdometerStatesAPIURL = "https://operations.cropwise.com/api/v3/odometer_states"

// FetchAndSaveOdometerStates - функция для загрузки и сохранения данных
func FetchAndSaveOdometerStates(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := OdometerStatesAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.OdometerState `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения odometer_state с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные odometer_states успешно загружены для схемы %s", schemaName)
	return nil
}
