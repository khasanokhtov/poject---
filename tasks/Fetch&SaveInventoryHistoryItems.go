...
const InventoryHistoryItemsAPIURL = "https://operations.cropwise.com/api/v3/inventory_history_items"

type InventoryHistoryItemsResponse struct {
	Data []models.InventoryHistoryItemModel `json:"data"`
	Meta struct {
		Response struct {
...
// FetchAndSaveInventoryHistoryItems - функция для загрузки и сохранения inventory_history_items
func FetchAndSaveInventoryHistoryItems(token string, schemaName string) error {
	log.Printf("Начинаем загрузку данных InventoryHistoryItems для схемы: %s", schemaName)
	...
	for {
		url := InventoryHistoryItemsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		var response InventoryHistoryItemsResponse
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения inventory_history_item с ID %d: %w", item.ID, err)
			}
		}
