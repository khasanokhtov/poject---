...
func FetchAndSaveFieldShapeLandParcelMappingItems(token string, schemaName string) error {
	log.Printf("Начинаем загрузку данных сопоставления участков земель и форм полей для схемы: %s", schemaName)
	...
	for {
		url := FieldShapeLandParcelMappingItemsAPIURL + "?from_id=" + strconv.Itoa(fromID)
