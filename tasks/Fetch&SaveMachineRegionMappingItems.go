...
type MachineRegionMappingItemsResponse struct {
	Data []models.MachineRegionMappingItem `json:"data"`
	Meta struct {
		Response struct {
...
func FetchAndSaveMachineRegionMappingItems(db *gorm.DB, token, schemaName string) error {
	...
	for {
		...
		for _, mappingItem := range response.Data {
			if err := db.Save(&mappingItem).Error; err != nil {
