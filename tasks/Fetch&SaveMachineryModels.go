...
const MachineryModelsAPIURL = "https://operations.cropwise.com/api/v3/machinery_models"

// FetchAndSaveMachineryModels - функция для загрузки и сохранения данных
func FetchAndSaveMachineryModels(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MachineryModelsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MachineryModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения machinery_model с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные machinery_models успешно загружены для схемы %s", schemaName)
	return nil
}
