...
func FetchAndSaveFertilizers(db *gorm.DB, token, schemaName string) error {
	log.Printf("Начинаем загрузку данных об удобрениях для схемы: %s", schemaName)
	...
	for {
		url := FertilizersAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		var response struct {
			Data []models.Fertilizer `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, fertilizer := range response.Data {
			if err := db.Save(&fertilizer).Error; err != nil {
