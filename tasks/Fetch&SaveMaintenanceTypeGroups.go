...
const MaintenanceTypeGroupsAPIURL = "https://operations.cropwise.com/api/v3/maintenance_type_groups"

// FetchAndSaveMaintenanceTypeGroups - функция для загрузки и сохранения данных
func FetchAndSaveMaintenanceTypeGroups(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MaintenanceTypeGroupsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MaintenanceTypeGroup `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения maintenance_type_group с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные maintenance_type_groups успешно загружены для схемы %s", schemaName)
	return nil
}
