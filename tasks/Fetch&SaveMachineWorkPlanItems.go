...
const MachineWorkPlanItemsAPIURL = "https://operations.cropwise.com/api/v3/machine_work_plan_items"

// FetchAndSaveMachineWorkPlanItems - функция для загрузки и сохранения данных
func FetchAndSaveMachineWorkPlanItems(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MachineWorkPlanItemsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MachineWorkPlanItemModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения machine_work_plan_item с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные machine_work_plan_items успешно загружены для схемы %s", schemaName)
	return nil
}
