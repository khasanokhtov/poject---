...
const MachineTaskGroupFolderMappingItemsAPIURL = "https://operations.cropwise.com/api/v3/machine_task_group_folder_mapping_items"

// FetchAndSaveMachineTaskGroupFolderMappingItems - функция для загрузки и сохранения данных
func FetchAndSaveMachineTaskGroupFolderMappingItems(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := MachineTaskGroupFolderMappingItemsAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.MachineTaskGroupFolderMappingItemModel `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения machine_task_group_folder_mapping_item с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные machine_task_group_folder_mapping_items успешно загружены для схемы %s", schemaName)
	return nil
}
