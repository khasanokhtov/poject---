...
const ProductionCyclesAPIURL = "https://operations.cropwise.com/api/v3/production_cycles"

// FetchAndSaveProductionCycles - функция для загрузки и сохранения данных
func FetchAndSaveProductionCycles(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := ProductionCyclesAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.ProductionCycle `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения production_cycle с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные production_cycles успешно загружены для схемы %s", schemaName)
	return nil
}
