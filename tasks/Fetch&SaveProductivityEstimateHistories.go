...
const ProductivityEstimateHistoriesAPIURL = "https://operations.cropwise.com/api/v3/productivity_estimate_histories"

// FetchAndSaveProductivityEstimateHistories - функция для загрузки и сохранения данных
func FetchAndSaveProductivityEstimateHistories(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := ProductivityEstimateHistoriesAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		// Парсим JSON-ответ
		var response struct {
			Data []models.ProductivityEstimateHistory `json:"data"`
			Meta struct {
				Response struct {
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения productivity_estimate_history с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные productivity_estimate_histories успешно загружены для схемы %s", schemaName)
	return nil
}
