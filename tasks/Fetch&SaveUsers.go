...
const UsersAPIURL = "https://operations.cropwise.com/api/v3a/users"

// FetchAndSaveUsers - функция для загрузки и сохранения данных
func FetchAndSaveUsers(token, schemaName string) error {
	...
	// Цикл для пагинации
	for {
		url := UsersAPIURL + "?from_id=" + strconv.Itoa(fromID)
		...
		for _, item := range response.Data {
			if err := database.DB.Save(&item).Error; err != nil {
				return fmt.Errorf("ошибка сохранения user с ID %d: %w", item.ID, err)
			}
		}
	...
	log.Printf("Данные users успешно загружены для схемы %s", schemaName)
	return nil
}
